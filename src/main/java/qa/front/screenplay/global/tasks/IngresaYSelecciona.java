package qa.front.screenplay.global.tasks;

import net.serenitybdd.core.steps.Instrumented;
import net.serenitybdd.screenplay.Actor;
import net.serenitybdd.screenplay.Task;
import net.serenitybdd.screenplay.actions.Enter;
import net.serenitybdd.screenplay.actions.Hit;
import net.serenitybdd.screenplay.waits.WaitUntil;
import org.openqa.selenium.Keys;
import qa.front.screenplay.global.elements.Element;

import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.isClickable;
import static net.serenitybdd.screenplay.matchers.WebElementStateMatchers.isVisible;

public class IngresaYSelecciona implements Task {

    private final String valor;
    private final String campo;

    public IngresaYSelecciona(String valor, String campo) {
        this.valor = valor;
        this.campo = campo;
    }

    public static IngresaYSelecciona en(String valor, String campo) {
        return Instrumented.instanceOf(IngresaYSelecciona.class).withProperties(valor, campo);
    }

    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(WaitUntil.the(Element.get(campo), isVisible()).forNoMoreThan(60).seconds());
        actor.attemptsTo(WaitUntil.the(Element.get(campo), isClickable()).forNoMoreThan(30).seconds());
        actor.attemptsTo(Enter.theValue(valor).into(Element.get(campo)));
        try {
            Thread.sleep(3 * 1000L);
        } catch (Exception e) {
            //tiempo de espera
        }
        actor.attemptsTo(Hit.the(Keys.ARROW_DOWN).into(Element.get(campo)));
        try {
            Thread.sleep(2 * 1000L);
        } catch (Exception e) {
            //tiempo de espera
        }
        actor.attemptsTo(Hit.the(Keys.ENTER).into(Element.get(campo)));
    }
}
